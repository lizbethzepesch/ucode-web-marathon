...
class WebSocketServer {
    ...
    public function startServer() {
        ...
        while (true) {
            ...
            foreach ($read as $connect) { // обрабатываем все соединения, в которых есть данные для чтения
                ...
                if($decoded['payload'] === "Stop server"){
                    $this->stopServer();
                }
                // получено сообщение от клиента, вызываем пользовательскую
                // функцию, чтобы обработать полученные данные
                if (is_callable($this->handler)) {
                    $socketKey = array_search($connect, $this->connects);
                    self::RequestController($decoded['payload'], $socketKey);
                }
            }
    ...
    private function RequestController($data, $socketKey){
        ...
        switch($dataArr['id']){
            case 1://login(id = 1, username, password)
                $username = $dataArr['username'];
                $pass = $dataArr['password'];
                $response = $this->db_connect->loginUser($username, $pass);
                if($response != NULL){
                ...
                {
                    self::response($this->connects[$socketKey], "Incorrect login or password!");
                }
                break;
            case 2://register(id = 2, $username, $password, $id_class
                
                $response = $this->db_connect->registerUser($dataArr['username'], $dataArr['password'], $dataArr['id_class']);
                if($response)
                {
                    ...
                    self::response($this->connects[$socketKey], "This user already exists!");
                }
                break;
            ...
            case 5://invited user response(id = 5, status(1 - yes, 0 - no), firstPlayer)
                if($dataArr['status'] == 1){
                    //create game 
                    self::response($this->connects[$dataArr['firstPlayer'] ], "User $socketKey accepted your invite");
                }
                else{
